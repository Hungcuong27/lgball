...
def api_withdraw():
    ...
    # Lưu lịch sử rút, cập nhật số dư (bạn có thể mở rộng models.py nếu muốn lưu tx_hash)
    add_transaction(address, 'withdraw', amount, status='success')
    decrease_balance(address, amount)  # <--- Thêm dòng này để trừ số dư
...
def open_chest():
    ...
    user = get_user(address)
    price = selected.get('price', 0.5) # fallback nếu không có trường price
